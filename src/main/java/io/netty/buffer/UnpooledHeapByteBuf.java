...
public class UnpooledHeapByteBuf extends AbstractReferenceCountedByteBuf {
    ...
    @Override
    public ByteBuf capacity(int newCapacity) {      // 扩容方法。线程不安全的方法
        checkNewCapacity(newCapacity);              // 检查参数合法

        int oldCapacity = array.length;
        byte[] oldArray = array;
        if (newCapacity > oldCapacity) {            // 扩容
            byte[] newArray = allocateArray(newCapacity);
            System.arraycopy(oldArray, 0, newArray, 0, oldArray.length);    // 堆内的byte数组，直接System.arraycopy复制
            setArray(newArray);
            freeArray(oldArray);
