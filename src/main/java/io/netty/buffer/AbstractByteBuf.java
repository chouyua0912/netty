...
/** 处理指针相关操作，具体使用DirectBuffer还是使用byte[]数组由子类决定，实际的数据操作在子类实现
...
public abstract class AbstractByteBuf extends ByteBuf {
    ...
    private int maxCapacity;                // 构造时候指定的最大大小，限制自动扩展，超过这个值的时候会抛出异常
    ...
    public ByteBuf discardReadBytes() {
        ensureAccessible();
        if (readerIndex == 0) {             // 还没有可重用的内存
            return this;
        }

        if (readerIndex != writerIndex) {   // readerIndex != 0, 并且还有可读取数据 -> 需要复制数据
            setBytes(0, this, readerIndex, writerIndex - readerIndex);  // 复制内存，移动指针，
            writerIndex -= readerIndex;
            adjustMarkers(readerIndex);     // 调整markedReaderIndex和markedWriterIndex
            readerIndex = 0;
        } else {                            // 没有可读取数据，直接移动指针即可
    ...
    public ByteBuf readBytes(ByteBuf dst, int dstIndex, int length) {
        checkReadableBytes(length);
        getBytes(readerIndex, dst, dstIndex, length);   // 随机读取
        readerIndex += length;                          // 处理指针操作
        return this;
    }
    ...
    public ByteBuf writeBytes(byte[] src, int srcIndex, int length) {
        ...
        setBytes(writerIndex, src, srcIndex, length);   // 从传入的byte数组的srcIndex开始读取length，写入当前ByteBuf
        writerIndex += length;
        return this;
    }
    ...
    protected final void checkNewCapacity(int newCapacity) {
        ensureAccessible();
        if (newCapacity < 0 || newCapacity > maxCapacity()) {       // 扩容容量参数校验
